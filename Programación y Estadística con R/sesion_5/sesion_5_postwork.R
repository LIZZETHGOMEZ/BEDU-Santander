    ...
    # Cargamos los datos
    B1.1920 <- "https://www.football-data.co.uk/mmz4281/1920/SP1.csv"
    B1.1819 <- "https://www.football-data.co.uk/mmz4281/1819/SP1.csv"
    B1.1718 <- "https://www.football-data.co.uk/mmz4281/1718/SP1.csv"
    
    # Guardamos en una lista para manipulacion y los leemos
    lista <- list(B1.1920,B1.1819,B1.1718)
    lista <- lapply(lista, read.csv)
    
    # Creamos el data frame con las variables de interes
    SmallData <- matrix(ncol = 5) 
    colnames(SmallData) <- c("Date", "HomeTeam", "AwayTeam", "FTHG", "FTAG")
    SmallData <- as.data.frame(SmallData)
    
    
    for(i in lista) {
        i <- i %>% select("Date", "HomeTeam":"FTAG")
        SmallData <- full_join(SmallData,i)
    }
    
    # Renombramos variables para el uso posterior de "fbRanks"
    SmallData <- SmallData %>% rename(date = Date,
                                      home.team = HomeTeam,
                                      away.team = AwayTeam,
                                      home.score = FTHG,
                                      away.score = FTAG)
    ...
    # Quitamos NAs
    SmallData <- SmallData[complete.cases(SmallData),]
    
    # Guardamos
    write.csv(SmallData,"data/soccer.csv", row.names = F)
